(function(root) {
  ...
  Poker.compareHands = function(hand1, hand2) {
    hand1['kicker'] = [];
	hand2['kicker'] = [];
    for (var i = 4; i >= 0; i--) {
	  hand1['kicker'].push(hand1['hand'][i].valueString);
	  hand2['kicker'].push(hand2['hand'][i].valueString);
	  if (hand1['hand'][i].value > hand2['hand'][i].value) {
	    return hand1;
	  } else if (hand1['hand'][i].value < hand2['hand'][i].value) {
	    return hand2;
	  }
	}
	return 'tie';
  ...
  Poker.checkKickers = function(handsArray) {
    var highHand = '';
	var winningPlayers = [];
    handsArray.forEach(function(hand) {
	  if (highHand === '') {
	    highHand = hand;
		winningPlayers.push(hand['player'].name)
	  } else {
	    var comparison = Poker.compareHands(highHand, hand);
	    if (comparison === 'tie') {
		  winningPlayers.push(hand['player']);
		} else {
		  highHand = comparison;
		  winningPlayers = [comparison['player']]
		}
	  }
	})
	var winners = []
	if (winningPlayers.length > 1) {
	  ...
	  var winnerString = winners.join(' and ');
	  winnerString += ' win with: ';
	} else {
	  var winnerString = winningPlayers[0].name;
	  winnerString += ' wins with: ';
	}
	
	var kickerString = highHand.join(', ') + ' kicker';
	
	winnerString += (highHand['string'] + ', ' + kickerString);
	
	return {'string': winnerString, 'winner': winningPlayers}
  ...
  Poker.evaluateHand = function(pocket, board) {
	...
	case 'Three of a Kind':
	  var kickArray = [aceValue(setHand[0]), aceValue(setHand[4]), aceValue(setHand[3])]
