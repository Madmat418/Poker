...
var newHand = function(socket, io, room, dealer) {
  if (playerRooms[room].length < 2) {
    io.sockets.in(room).emit('message', {
      text: "Requires two or more players to be seated at this table; if you would like to test gameplay, join with a second window.",
      room: room
    })
    socket.emit('gameNotStarted')
  } else {
...
var foldWin = function(socket, io) {
  socket.on('winByFold', function(data) {
    ...
    updateInfo(data.room, game.players[0], socket, io);
    setTimeout(function(){newHand(socket, io, data.room, findNextDealer(game.dealer + 1, game.room), 4000)});
    delete currentGame(data.room);
  })
}
...
var handleDisconnection = function(socket, io) {
  socket.on('disconnect', function() {
    ...
    if (game) {
      ...
      if (currentName === name) {
        current = true;
      } else if (game.currentPlayer > leavingPlayer.order) {
        game.currentPlayer -= 1;
      }
      game.numPlayers -= 1;
